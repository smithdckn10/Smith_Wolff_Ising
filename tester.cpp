...
void artOut(int spins[], int checked[]); 
...
int main(){
    ...
    double Jvalues[numberOfTemps] = {0.08};
    double Jval = 0.01;
...
void artOut(int spins[], int checked[]){
    ...
    for(int i = 0; i < L; i++){
        for(int j = 0; j < L; j++){
            ...
            /*     //Enable this if you want to show the positions as well
            space = "     ";
...
void wolf(int initPos, int spins[], int neighborPos[], int checked[], double Jval){
    //each site has an attribute checked, so -1 = unchecked, 0 = checked and not in, 1 = in the cluster, -2 = should be checked at some point
    double chance;
    double prob;
    checked[initPos] = 1;
    cout << "initialPositionIs: " << initPos << ", checked[initPos] = " << checked[initPos] << endl;
    ...
    if(checked[t] == checked[b] == checked[l] == checked[r] == 0){
        artOut(spins, checked);
        //cout << "Dead End. Next Branch: ";
    }
